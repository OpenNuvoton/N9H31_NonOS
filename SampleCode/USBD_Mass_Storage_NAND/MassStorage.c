...
extern uint32_t volatile g_u8InitFlag;
...
void MSC_Init(void)
{
    ...
    if (g_u8InitFlag)
    {
        g_TotalSectors = ptNDisk->nLBlockCount * ptNDisk->nPagePerBlock * ptNDisk->nPageSize / 512;
        g_au8SenseKey[0] = 0x0;
        g_au8SenseKey[1] = 0x0;
        g_au8SenseKey[2] = 0x0;
    }
...
{
    //sysprintf("msc read: addr %d, size %d\n", addr, size);
    NANDLIB_Read(ptNDisk, addr, size, buffer);
}

void MSC_WriteMedia(uint32_t addr, uint32_t size, uint8_t *buffer)
{
    //sysprintf("msc write: addr %d, size %d\n", addr, size);
    NANDLIB_Write(ptNDisk, addr, size, buffer);
}
